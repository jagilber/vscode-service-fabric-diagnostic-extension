...
client.on('error', (error: Error) => {
		...
		return this.connect().then(client => {
			return new Promise((c, e) => {
				client.list((err: any, list: any) => {
					...
					return c(this.sort(list.map((entry: any) => ({ resource: vscode.Uri.parse(`ftp://${this.host}///${entry.name}`), isDirectory: entry.type === 'd' }))));
				});
			});
		});
	}
		...
		return this.connect().then(client => {
			return new Promise((c, e) => {
				client.list(node.resource.fsPath, (err: any, list: any) => {
					...
					return c(this.sort(list.map((entry: any) => ({ resource: vscode.Uri.parse(`${node.resource.fsPath}/${entry.name}`), isDirectory: entry.type === 'd' }))));
				});
			});
		});
	}
	...
	public getContent(resource: vscode.Uri): Thenable<string> {
		return this.connect().then(client => {
			return new Promise((c, e) => {
				client.get(resource.path.substr(2), (err: any, stream: any) => {
					if (err) {
						return e(err);
					}
