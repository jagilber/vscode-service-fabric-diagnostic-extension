...
namespace _ {
	...
	export function writefile(path: string, content: Uint8Array): Promise<void> {
		return new Promise<void>((resolve, reject) => {
			fs.writeFile(path, content as any, (error: NodeJS.ErrnoException | null) => handleResult(resolve, reject, error, void 0));
		});
	}
	...
	export function rmrf(path: string): Promise<void> {
		return new Promise<void>((resolve, reject) => {
			rimraf(path, (error: any) => handleResult(resolve, reject, error, void 0));
		});
	}

	export function mkdir(path: string): Promise<void> {
		return new Promise<void>((resolve, reject) => {
			mkdirp(path).then(() => handleResult(resolve, reject, null, void 0)).catch((error: any) => handleResult(resolve, reject, error, void 0));
		});
	}
...
export class FileSystemProvider implements vscode.TreeDataProvider<Entry>, vscode.FileSystemProvider {
	...
	readFile(uri: vscode.Uri): Uint8Array | Thenable<Uint8Array> {
		return _.readfile(uri.fsPath).then(buffer => Uint8Array.from(buffer));
	}
